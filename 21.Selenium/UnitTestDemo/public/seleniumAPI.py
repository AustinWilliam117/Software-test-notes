...
class browser(object):
    ...
    def explicit_wait(self,by,value,secs=10):
        """
        显示等待
        """
        try:
    ...
    def get_element(self,css):
        ...
        """
        if "=>" not in css:
            by = "css"  # 如果是css的格式是#aaa,所以在此加入判断如果不包含=>就默认是css传给上面的explicit_wait判断元素是否存在
            value = css
            # wait element.
            self.explicit_wait(by, css)
        else:
            by = css.split("=>")[0]
            value = css.split("=>")[1]
            if by == "" or value == "":
